...
export default function SalesPage() {
  ...
  const salesTimeSeries = useMemo(() => {
    const map = new Map<
      string,
      { date: string; dine_in: number; takeout: number; delivery: number }
    >();

    for (const s of sales) {
      if (
        selectedLocation &&
        s.LOCATION_ID !== selectedLocation.LOCATION_ID
      ) {
        continue;
      }
      const entry =
      ...
      const key =
        s.ORDER_TYPE === "dine-in"
          ? "dine_in"
          : (s.ORDER_TYPE as "takeout" | "delivery");
      entry[key] += Number(s.REVENUE);
      map.set(s.SALE_DATE, entry);
    }
  ...
  const ordersAndAovTimeSeries = useMemo(() => {
    const map = new Map<
      string,
      { date: string; revenue: number; orders: number; aov: number }
    >();
    for (const s of sales) {
      if (
        selectedLocation &&
      ...
      entry.orders += Number(s.NUM_ORDERS);
      map.set(s.SALE_DATE, entry);
    }
  ...
  const orderTypeMix = useMemo(() => {
    ...
    for (const s of sales) {
      ...
      revenue[s.ORDER_TYPE] += Number(s.REVENUE);
      orders[s.ORDER_TYPE] += Number(s.NUM_ORDERS);
    }
    const totalRevenue =
      revenue["dine-in"] + revenue.takeout + revenue.delivery;

    return (["dine-in", "takeout", "delivery"] as const).map(
      (k) => ({
        key: k,
        name:
          k === "dine-in"
        ...
        revenue: revenue[k],
        orders: orders[k],
        share: totalRevenue ? revenue[k] / totalRevenue : 0,
      })
    );
  }, [sales, selectedLocation]);

  const revenueByLocationChart = useMemo(() => {
    const totals = new Map<number, number>();
    for (const s of sales) {
      totals.set(
        s.LOCATION_ID,
        (totals.get(s.LOCATION_ID) ?? 0) + Number(s.REVENUE)
      );
    }
    return locations
      .map((loc) => ({
        ...
        name: loc.NAME,
        revenue: totals.get(loc.LOCATION_ID) ?? 0,
      }))
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 px-4 md:px-6 py-4 md:py-6 max-w-7xl w-full mx-auto space-y-4 md:space-y-6">
        ...
        <Card>
          <CardContent className="px-2">
            <div className="grid grid-cols-2 md:grid-cols-4 text-xs md:text-sm">
              <Link
                to="/sales"
                className="px-4 py-3 border-r border-b md:border-b-0 bg-muted text-foreground transition-colors"
              >
                Sales & orders
              </Link>
              <Link
        ...
        <section className="space-y-4 md:space-y-6" aria-label="Sales and orders">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
            <Card className="lg:col-span-8 bg-gradient-to-br from-sky-500/10 via-background to-background">
              <CardHeader>
                <CardTitle>Sales over time</CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Revenue trend by order type for{" "}
            ...
            <Card className="lg:col-span-4 bg-gradient-to-br from-violet-500/10 via-background to-background">
              <CardHeader>
                <CardTitle>Order type mix</CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Revenue share and order volume by channel.
                </p>
              </CardHeader>
              <CardContent className="h-[360px] md:h-[420px] flex flex-col gap-3">
                {orderTypeMix.every((x) => x.revenue === 0) ? (
                  <div className="h-full flex flex-col gap-3 p-4">
                    <Skeleton className="flex-1 rounded-full mx-auto w-[200px]" />
                  ...
                  <>
                        ...
                        <PieChart>
                          <Tooltip
                            formatter={(v: unknown, name: unknown) => {
                              if (name === "Orders") {
                                return [numberTooltipFormatter(v), "Orders"];
                              }
                              return [
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {orderTypeMix.map((row, idx) => (
                        <div
                          key={row.key}
                          className="rounded-lg border border-border/70 bg-muted/40 px-2.5 py-2"
                        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
            <Card className="lg:col-span-7 bg-gradient-to-br from-emerald-500/10 via-background to-background">
              <CardHeader>
                <CardTitle>Revenue by location</CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Who's driving the business right now (selected date range).
                </p>
              </CardHeader>
              <CardContent className="h-[320px] md:h-[380px]">
                {revenueByLocationChart.length === 0 ? (
                  <div className="h-full flex flex-col gap-3 p-4">
                    <Skeleton className="h-8 w-32" />
                    <Skeleton className="h-full w-full" />
                  </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={revenueByLocationChart.slice(0, 12)}
            ...
            <Card className="lg:col-span-5 bg-gradient-to-br from-amber-500/10 via-background to-background">
              <CardHeader>
                <CardTitle>Orders & average ticket</CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Daily order volume and average order value (AOV).
                </p>
              </CardHeader>
              <CardContent className="h-[320px] md:h-[380px]">
                {ordersAndAovTimeSeries.length === 0 ? (
                  <div className="h-full flex flex-col gap-3 p-4">
                    <Skeleton className="h-8 w-32" />
                    <Skeleton className="h-full w-full" />
                  </div>
